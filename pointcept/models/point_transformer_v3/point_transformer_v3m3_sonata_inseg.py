...
class Embedding(PointModule):
    def __init__(
        ...
        self.query_embed = nn.Sequential(nn.Linear(1, embed_channels), norm_layer(embed_channels), act_layer())
        nn.init.zeros_(self.query_embed[0].weight)
        nn.init.zeros_(self.query_embed[0].bias)

        if norm_layer is not None:
            self.query_embed.append(norm_layer(embed_channels))
        if act_layer is not None:
            self.query_embed.append(act_layer())
    ...
    def forward(self, point: Point):
        ...
        point.feat = point.feat + self.query_embed(point.query_mask.float())
