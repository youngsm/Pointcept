...
def _distributed_worker(
    ...
    try:
        ...
        if "address already in use" in str(e).lower():
            logger.error(
                "Port is already in use. You can fix this by:\n"
                "1. Set a random port before running: export MASTER_PORT=$(shuf -i 20000-30000 -n 1)\n"
                "2. Kill any stale processes that may be using the port\n"
                "3. Use dist_url=\"auto\" which will try to find an available port"
            )
        elif "connection refused" in str(e).lower() or "connect" in str(e).lower():
            logger.error(
                "Connection error. Check the following:\n"
                "1. Ensure all nodes can communicate with each other (try ping)\n"
                "2. Check firewall rules between nodes\n"
                "3. Make sure master node is reachable at the specified address\n"
                "4. Try setting NCCL_SOCKET_IFNAME to the correct network interface\n"
                "   Current: NCCL_SOCKET_IFNAME=" + os.environ.get('NCCL_SOCKET_IFNAME', 'not set')
            )
        elif "timeout" in str(e).lower():
            logger.error(
                "Timeout error. Try increasing timeouts:\n"
                "1. Set NCCL_IB_TIMEOUT=30\n"
                "2. Set NCCL_SOCKET_NTHREADS=8\n"
                "3. Set TORCH_DISTRIBUTED_TIMEOUT to a higher value"
            )
