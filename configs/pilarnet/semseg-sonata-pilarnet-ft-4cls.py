...
wandb_run_name = f"sonata-pilarnet-semseg-ft-v1-4GPU-4cls-{num_events_train}ev-256patch"  # Descriptive name for this run
...
model = dict(
    ...
    criteria=[
        # dict(type="BinaryFocalLoss", loss_weight=1.0, weight=class_weights, ignore_index=-1),
        dict(type="CrossEntropyLoss", loss_weight=1.0, weight=class_weights, ignore_index=-1),
...
data = dict(
    num_classes=4,
    ignore_index=-1,
    names=['track', 'shower', 'michel', 'delta'],
    train=dict(
        type="ConcatDataset",
        datasets=[
            # PILArNet
...
hooks = [
    ...
    dict(type="SemSegEvaluator", every_n_epochs=10, write_cls_iou=True),
    dict(type="CheckpointSaver", save_freq=None, evaluator_every_n_epochs=10),
